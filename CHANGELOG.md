...
- Graded defense utility penalties in `planner.rs` and `enforcement.rs` (`plan_snapshot_with_defense_penalties`, `defense_aware_plan_with_penalties`, and `UtilityBreakdown::penalized`), generalizing binary suppression to basis-point score scaling (`MAX_BASIS_POINTS = 10,000`) while preserving default-off enforcement discipline.
