...
REST read-only review is a separate non-default child of that same runtime.
It requires explicit `web-review` plus same-run `openapi-review`; compiling or
passing `rest-review` never silently enables OpenAPI work. Only a complete,
replay-stable OpenAPI catalog can yield one eligible anonymous, bodyless,
exact-origin `GET` with no required inputs. The child sends exactly candidate
and replay through the shared broker and budget, charges one logical active
verification, and requires stable `Status`, `Fields`, and `Resources` before
emitting `api.rest-readonly-surface-observed@1` as `Informational` /
`KnowledgeOnly`. It executes no described write, materializes no parameter or
