...
pack_id = "termivar-canary"
pack_revision = 1
title = "Termivar canary"
...
"#
    ...
    fn strict_pack_parsing_normalizes_order_and_tags() {
        ...
        assert_eq!(pack.id().as_str(), "termivar-canary");
    ...
    fn catalog_digest_is_semantic_and_order_independent() {
        ...
        assert_eq!(
            first.digest().as_str(),
            "artifact-catalog-sha256:6e7c672623b1b714924c1a9e5989dd96e66357fa703893226739191dad75c3d7"
        );
    }
    ...
    fn exact_lookup_and_bounded_filters_are_deterministic() {
        ...
        let reference = ArtifactSignatureRef::new(
            ArtifactPackId::parse("termivar-canary").expect("pack id"),
    ...
    fn strict_schema_identity_revision_text_and_input_bounds() {
        ...
        for replacement in [
            ...
            let source = manifest(true).replace("pack_id = \"termivar-canary\"", replacement);
            assert!(matches!(
    ...
    fn free_form_manifest_metadata_never_enters_public_debug() {
        ...
        let source = manifest(true)
            .replace("Termivar canary", SENTINEL)
...
tags = ["scale"]"#,
    ...
    fn identity_revision_metadata_tag_and_list_limits_are_exact() {
        ...
        for (old, replacement) in [
            ("Termivar canary", "x".repeat(MAX_PACK_TITLE_BYTES + 1)),
            (
    ...
    fn cross_pack_patterns_pack_capacity_and_query_results_are_bounded() {
        ...
        let duplicate_pattern = ArtifactSignaturePack::parse_toml(
            manifest(true)
                .replace("pack_id = \"termivar-canary\"", "pack_id = \"other-pack\"")
                .as_bytes(),
        )
        .expect("other pack");
