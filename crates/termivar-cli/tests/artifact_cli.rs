...
const LAB_SIGNATURES: &str = include_str!(concat!(
    ...
    "/../../artifact-signatures/lab/termivar-canary/signatures.toml"
));
...
fn json_scan_reports_observations_without_path_or_matched_bytes() {
    ...
    let (signatures, input) = write_inputs(directory.path(), b"prefix-TERMIVAR-CANARY-suffix");
    ...
    assert!(!rendered.contains("TERMIVAR-CANARY"));
...
fn text_output_is_bounded_and_observation_only() {
    let directory = tempfile::tempdir().unwrap();
    let (signatures, input) = write_inputs(directory.path(), b"TERMIVAR-CANARY");
    ...
    assert!(!stdout.contains("TERMIVAR-CANARY"));
...
"#;
    ...
    assert!(diagnostic.contains("signature manifest is invalid"));
    assert!(!diagnostic.contains("termivar-canary-exact"));
}
...
fn input_above_the_default_bound_is_rejected_without_reading_or_reporting_it() {
    ...
    let input_file = std::fs::OpenOptions::new()
        ...
        .unwrap();
    input_file
        .set_len(termivar_artifact::DEFAULT_INPUT_BYTES + 1)
        .unwrap();
    drop(input_file);
...
fn match_limit_returns_a_typed_partial_report_and_nonzero_exit() {
    let directory = tempfile::tempdir().unwrap();
    let input_bytes = vec![b'A'; termivar_artifact::DEFAULT_MATCHES_PER_SCAN + 2];
    ...
    assert_eq!(
        report["match_count"],
        termivar_artifact::DEFAULT_MATCHES_PER_SCAN
    );
    let diagnostic = String::from_utf8(output.stderr).unwrap();
