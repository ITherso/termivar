...
fn scan_wordpress_review_flags_conflict(
    ...
    } else if selected.context && saved_inventory_selected {
...
struct ScanArgs {
    ...
    /// Select the declared schema of `--wordpress-advisories`. The default is
    /// Termivar's strict native catalogue; external formats are never inferred
    /// from a filename or document contents.
    #[cfg(feature = "wordpress-review")]
    #[arg(
        long,
        value_enum,
    ...
    /// Read the V2 supplied session's bounded cookie-id/value rows from a
    /// regular file. The path and values are redacted; browser profiles are
    /// never searched or imported.
    #[cfg(feature = "supplied-session-review")]
    #[arg(
        long,
        value_name = "FILE",
    ...
    fn supplied_session_has_only_explicit_web_review_secret_sources() {
        ...
        for forbidden in [
            ...
            assert!(
                ...
                "unexpected supplied-session secret source {forbidden}"
            );
        }
        ...
        let cookie_review = Cli::try_parse_from([
            "termivar",
            "scan",
            "--profile",
            "web-review",
        ...
        .unwrap();
        let cookie_review = parsed_scan_args(&cookie_review);
