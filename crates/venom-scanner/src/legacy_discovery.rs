//! Bounded transport and typed state for the opt-in legacy discovery phases.
//!
//! This is a migration boundary, not a second scanner runtime. Phases two
//! through four share this authority while the remaining historical phases
//! still use the explicitly unmetered legacy client.
...
impl LegacyDiscoveryAuthority {
    ...
    pub(crate) fn canonicalize(&self, url: &Url) -> Result<Url, ScannerError> {
        if !matches!(url.scheme(), "http" | "https")
            ...
            || url.origin() != self.authorized_origin
        {
...
fn map_broker_error(error: HttpRequestBrokerError) -> ScannerError {
    match error {
        ...
        HttpRequestBrokerError::Http(_) => ScannerError::NetworkError(
            "bounded discovery transport failed without accepting a result".to_owned(),
        ),
    }
}
