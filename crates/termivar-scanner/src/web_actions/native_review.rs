...
#[cfg(feature = "scanning")]
pub(crate) fn native_web_review_response_marker_predicate() -> termivar_core::KnowledgePredicate {
...
pub(crate) fn authorization_review_phase_terminal_predicate() -> termivar_core::KnowledgePredicate {
    termivar_core::KnowledgePredicate::new("web.authorization-review.transport", "phase-terminal")
        .expect("the authorization terminal predicate is a valid static identity")
}

#[cfg(all(feature = "scanning", feature = "openapi-review"))]
pub(crate) fn openapi_review_phase_terminal_predicate() -> termivar_core::KnowledgePredicate {
    termivar_core::KnowledgePredicate::new("web.openapi-review.transport", "phase-terminal")
        .expect("the OpenAPI terminal predicate is a valid static identity")
}

#[cfg(all(feature = "scanning", feature = "rest-review"))]
pub(crate) fn rest_review_catalog_ready_predicate() -> termivar_core::KnowledgePredicate {
    termivar_core::KnowledgePredicate::new("web.openapi-review.transport", "rest-catalog-ready")
        .expect("the REST catalog-ready predicate is a valid static identity")
}

#[cfg(all(feature = "scanning", feature = "rest-review"))]
pub(crate) fn rest_review_phase_terminal_predicate() -> termivar_core::KnowledgePredicate {
    termivar_core::KnowledgePredicate::new("web.rest-review.transport", "phase-terminal")
...
pub enum NativeWebReviewActionKind {
    ...
    /// Classify one inert scanner-owned marker in a bounded HTML parse tree.
    ...
    /// First matched safe arithmetic template-expression comparison.
...
impl NativeWebReviewActionKind {
    ...
    pub const fn all() -> [Self; NATIVE_WEB_REVIEW_ACTION_COUNT] {
        [
            ...
            Self::ReflectionContextQueryPair,
    ...
    pub const fn action_id(self) -> &'static str {
        match self {
            ...
            Self::ReflectionContextQueryPair => "web.review.reflection.context-query-pair@1",
    ...
    pub const fn executor_id(self) -> &'static str {
        match self {
            ...
            Self::ReflectionContextQueryPair => "web.review.probe.reflection-context-query-pair@1",
            ...
            Self::SstiStructuralQueryPair => "web.review.probe.ssti-structural-query-pair@1",
            Self::SstiStructuralQueryReplayPair => {
    ...
    pub const fn slug(self) -> &'static str {
        match self {
            ...
            Self::ReflectionContextQueryPair => "reflection-context-query-pair",
    ...
    pub fn risk(self) -> RiskScore {
        let percent = match self {
            ...
            Self::ReflectionContextQueryPair => 5,
            Self::SqlStructuralQueryPair
            | Self::SqlStructuralQueryReplayPair
            | Self::SstiStructuralQueryPair
            | Self::SstiStructuralQueryReplayPair
...
mod tests {
    ...
    fn native_action_identities_are_stable_unique_and_deterministic() {
        let expected = [
            ...
            (
                ...
                "web.review.reflection.context-query-pair@1",
                "web.review.probe.reflection-context-query-pair@1",
                "reflection-context-query-pair",
            ),
            (
            ...
            (
                ...
                "web.review.ssti.structural-query-pair@1",
                "web.review.probe.ssti-structural-query-pair@1",
                "ssti-structural-query-pair",
            ),
            (
                NativeWebReviewActionKind::SstiStructuralQueryReplayPair,
                "web.review.ssti.structural-query-replay-pair@1",
                "web.review.probe.ssti-structural-query-replay-pair@1",
    ...
    fn request_relationship_is_exactly_passive_control_then_active_candidate() {
        ...
        assert_eq!(
            NativeWebReviewActionKind::ReflectionContextQueryPair.differential_input(),
            NativeWebReviewDifferentialInput::SingleQueryParameter
        );
    }
