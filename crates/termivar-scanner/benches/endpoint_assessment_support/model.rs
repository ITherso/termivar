...
impl BenchmarkEnvironment {
    pub(crate) fn observed() -> Self {
        Self {
            commit_sha: env_or_unknown("TERMIVAR_PERF_COMMIT_SHA"),
            rust_version: env_or_unknown("TERMIVAR_PERF_RUST_VERSION"),
            os: env::var("TERMIVAR_PERF_OS").unwrap_or_else(|_| env::consts::OS.to_owned()),
            architecture: env::consts::ARCH,
            build_profile: env::var("TERMIVAR_PERF_BUILD_PROFILE")
                .unwrap_or_else(|_| "bench".to_owned()),
            package_version: env!("CARGO_PKG_VERSION"),
            hardware: HardwareEnvironment {
                cpu_model: env_or_unknown("TERMIVAR_PERF_CPU_MODEL"),
                logical_cpus: std::thread::available_parallelism()
                    .map(std::num::NonZeroUsize::get)
                    .unwrap_or(1),
                total_memory_bytes: env::var("TERMIVAR_PERF_TOTAL_MEMORY_BYTES")
                    .ok()
