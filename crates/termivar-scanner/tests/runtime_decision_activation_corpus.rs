//! Activation corpus for the deterministic `termivar scan` preview runtime.
//!
...
//! snapshot of the current `termivar scan` preview profile** (also exposed through
//! the deprecated `decision-scan` alias; see
//! `preview_runtime` below and `crates/termivar-cli/src/decision_scan.rs`) against
...
use termivar_core::{EvidenceValue, HypothesisState, HypothesisStrength, OutcomeStatus};
...
// current `termivar scan` preview profile in
// `crates/termivar-cli/src/decision_scan.rs`. They are not shared code and are not
