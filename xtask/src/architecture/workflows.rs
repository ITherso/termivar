...
const RECON_CT_PROVIDER_SMOKE_GATE: &str = r#"      - name: Exercise opt-in Cert Spotter CT provider CLI
        run: cargo test --release --locked -p termivar-cli --no-default-features --features recon-ct-provider --test recon_ct_provider_cli -- --nocapture"#;
const RECON_CT_PROVIDER_CAPABILITIES_SMOKE_GATE: &str = r#"      - name: Exercise opt-in Cert Spotter CT provider capability contract
        env:
          TERMIVAR_CAPABILITIES_MATRIX_CASE: recon-ct-provider-only
        run: cargo test --release --locked -p termivar-cli --no-default-features --features recon-ct-provider --test capabilities_cli matrix_case_proves_release_bundle_is_composition_not_origin -- --nocapture"#;
const RECON_SNAPSHOT_IMPORT_SMOKE_GATE: &str = r#"      - name: Exercise opt-in reconnaissance snapshot import CLI
        run: cargo test --release --locked -p termivar-cli --no-default-features --features recon-snapshot-import --test recon_snapshot_cli -- --nocapture"#;
const RECON_SNAPSHOT_IMPORT_CAPABILITIES_SMOKE_GATE: &str = r#"      - name: Exercise opt-in reconnaissance snapshot capability contract
        env:
...
fn capabilities_workflow_policy_violations(files: &[(String, String)]) -> Vec<String> {
    ...
    if !job_has_exact_step(
        &normalized,
        "platform-runtime-smoke",
        "Exercise opt-in passive secret-exposure review CLI",
        SECRET_EXPOSURE_SMOKE_GATE,
    ) {
        violations.push(format!(
    ...
    if !job_has_exact_step(
        &normalized,
        "platform-runtime-smoke",
        "Exercise opt-in local JWT policy review CLI",
        JWT_POLICY_REVIEW_SMOKE_GATE,
    ) {
        violations.push(format!(
            "{TESTS_WORKFLOW}: four-platform runtime smoke must compile and run the exact feature-minimal local JWT-policy CLI integration test"
        ));
    }
    if !job_has_exact_step(
        &normalized,
        "platform-runtime-smoke",
        "Exercise opt-in JWT target-acceptance review CLI",
        JWT_TARGET_ACCEPTANCE_REVIEW_SMOKE_GATE,
    ) {
        violations.push(format!(
    ...
    if !job_has_exact_step(
        &normalized,
        "platform-runtime-smoke",
        "Exercise opt-in Cert Spotter CT provider CLI",
        RECON_CT_PROVIDER_SMOKE_GATE,
    ) {
        violations.push(format!(
            "{TESTS_WORKFLOW}: four-platform runtime smoke must compile and run the exact release-profile feature-minimal Cert Spotter CT provider CLI integration test"
        ));
    }
    if !job_has_exact_step(
        &normalized,
        "platform-runtime-smoke",
        "Exercise opt-in Cert Spotter CT provider capability contract",
        RECON_CT_PROVIDER_CAPABILITIES_SMOKE_GATE,
    ) {
        violations.push(format!(
            "{TESTS_WORKFLOW}: four-platform runtime smoke must validate the exact release-profile feature-minimal Cert Spotter CT provider capability contract"
        ));
    }
    if !job_has_exact_step(
        &normalized,
        "platform-runtime-smoke",
        "Exercise opt-in reconnaissance snapshot import CLI",
        RECON_SNAPSHOT_IMPORT_SMOKE_GATE,
    ) {
        violations.push(format!(
            "{TESTS_WORKFLOW}: four-platform runtime smoke must compile and run the exact release-profile feature-minimal recon-snapshot-import CLI integration test"
        ));
    }
    if !job_has_exact_step(
        &normalized,
        "platform-runtime-smoke",
        "Exercise opt-in reconnaissance snapshot capability contract",
        RECON_SNAPSHOT_IMPORT_CAPABILITIES_SMOKE_GATE,
    ) {
        violations.push(format!(
    ...
    if !job_has_exact_step(
        &normalized,
        "platform-runtime-smoke",
        "Reject authenticated session bodies from the anonymous secret-exposure context",
        SECRET_EXPOSURE_SESSION_BOUNDARY_SMOKE_GATE,
    ) {
        violations.push(format!(
            "{TESTS_WORKFLOW}: four-platform runtime smoke must enforce the exact anonymous secret-exposure versus supplied-session boundary"
        ));
    }
    if !job_has_exact_step(
        &normalized,
...
fn progress_workflow_policy_violations(files: &[(String, String)]) -> Vec<String> {
    let Some((_, contents)) = files.iter().find(|(path, _)| path == TESTS_WORKFLOW) else {
        return vec![format!(
            "{TESTS_WORKFLOW}: reviewed live-progress runtime-smoke workflow is missing"
        )];
    };
    let normalized = contents.replace("\r\n", "\n");
    if job_has_exact_step(
        &normalized,
        "platform-runtime-smoke",
        "Exercise opt-in live progress CLI",
        PROGRESS_SMOKE_GATE,
    ) {
        Vec::new()
    } else {
...
mod tests {
    ...
    #[test]
    fn repository_live_progress_smoke_is_exact_on_all_platforms() {
        let contents = include_str!("../../../.github/workflows/tests.yml");
        for fixture in [contents.to_owned(), contents.replace('\n', "\r\n")] {
            let violations =
    ...
    #[test]
    fn live_progress_smoke_rejects_omission_substitution_and_suppression() {
        let valid = include_str!("../../../.github/workflows/tests.yml").replace("\r\n", "\n");
        for mutation in [
            valid.replacen(PROGRESS_SMOKE_GATE, "", 1),
            valid.replacen(
                PROGRESS_SMOKE_GATE,
                "      - name: Exercise opt-in live progress CLI\n        run: cargo test --locked -p termivar-cli --test profile_scan_cli",
                1,
            ),
            valid.replacen(
                PROGRESS_SMOKE_GATE,
                &format!("{PROGRESS_SMOKE_GATE}\n        continue-on-error: true"),
                1,
            ),
        ] {
            assert_ne!(mutation, valid, "mutation must alter the workflow fixture");
            let violations =
                progress_workflow_policy_violations(&[(TESTS_WORKFLOW.to_owned(), mutation)]);
            assert_eq!(violations.len(), 1, "{violations:?}");
            assert!(violations[0].contains("live-progress"), "{violations:?}");
        }
    }
    ...
    #[test]
    fn jwt_target_capabilities_matrix_rejects_omission_and_policy_aliasing() {
        let valid = include_str!("../../../.github/workflows/tests.yml").replace("\r\n", "\n");
        let jwt_target_case = "TERMIVAR_CAPABILITIES_MATRIX_CASE=jwt-target-only cargo test --locked -p termivar-cli --no-default-features --features jwt-target-acceptance-review --test capabilities_cli matrix_case_proves_release_bundle_is_composition_not_origin -- --nocapture";
        for mutation in [
            valid.replacen(jwt_target_case, "", 1),
            valid.replacen(
                jwt_target_case,
                &jwt_target_case.replacen(
            ...
            valid.replacen(
                jwt_target_case,
                &jwt_target_case.replacen(
                    "TERMIVAR_CAPABILITIES_MATRIX_CASE=jwt-target-only",
                    "TERMIVAR_CAPABILITIES_MATRIX_CASE=jwt-only",
                    1,
                ),
                1,
            ),
    ...
    #[test]
    fn recon_snapshot_import_capabilities_matrix_rejects_omission_and_aliasing() {
        let valid = include_str!("../../../.github/workflows/tests.yml").replace("\r\n", "\n");
        let recon_case = "TERMIVAR_CAPABILITIES_MATRIX_CASE=recon-snapshot-import-only cargo test --locked -p termivar-cli --no-default-features --features recon-snapshot-import --test capabilities_cli matrix_case_proves_release_bundle_is_composition_not_origin -- --nocapture";
        for mutation in [
            valid.replacen(recon_case, "", 1),
            valid.replacen(
                recon_case,
                &recon_case.replacen(
    ...
    #[test]
    fn recon_ct_provider_capabilities_matrix_rejects_omission_and_aliasing() {
        let valid = include_str!("../../../.github/workflows/tests.yml").replace("\r\n", "\n");
        let provider_case = "TERMIVAR_CAPABILITIES_MATRIX_CASE=recon-ct-provider-only cargo test --locked -p termivar-cli --no-default-features --features recon-ct-provider --test capabilities_cli matrix_case_proves_release_bundle_is_composition_not_origin -- --nocapture";
        for mutation in [
            valid.replacen(provider_case, "", 1),
            valid.replacen(
                provider_case,
                &provider_case.replacen(
                    "--features recon-ct-provider",
                    "--features release-bundle",
                    1,
                ),
                1,
    ...
    #[test]
    fn recon_ct_provider_feature_boundaries_reject_omission_and_bundle_substitution() {
        let valid = include_str!("../../../.github/workflows/tests.yml").replace("\r\n", "\n");
        let cli_case =
            "cargo test --locked -p termivar-cli --no-default-features --features recon-ct-provider";
        let scanner_member = "jwt-target-acceptance-review control-reference-mapping recon-ct-provider recon-snapshot-import legacy-scanner";
    ...
    #[test]
    fn jwt_target_acceptance_feature_boundaries_reject_omission_bundle_and_policy_aliasing() {
        let valid = include_str!("../../../.github/workflows/tests.yml").replace("\r\n", "\n");
    ...
    #[test]
    fn secret_exposure_runtime_smoke_rejects_omission_substitution_and_suppression() {
        let valid = include_str!("../../../.github/workflows/tests.yml").replace("\r\n", "\n");
        for mutation in [
            valid.replacen(SECRET_EXPOSURE_SMOKE_GATE, "", 1),
            valid.replacen(
                SECRET_EXPOSURE_SMOKE_GATE,
                "      - name: Exercise opt-in passive secret-exposure review CLI\n        run: cargo test --locked -p termivar-cli --all-features --test secret_exposure_cli -- --nocapture",
                1,
            ),
            valid.replacen(
                SECRET_EXPOSURE_SMOKE_GATE,
                &format!("{SECRET_EXPOSURE_SMOKE_GATE}\n        continue-on-error: true"),
                1,
            ),
            valid.replacen(SECRET_EXPOSURE_SESSION_BOUNDARY_SMOKE_GATE, "", 1),
            valid.replacen(
                SECRET_EXPOSURE_SESSION_BOUNDARY_SMOKE_GATE,
                "      - name: Reject authenticated session bodies from the anonymous secret-exposure context\n        run: cargo test --locked -p termivar-cli --no-default-features --features secret-exposure-review --test supplied_session_cli passive_secret_review_does_not_select_authenticated_session_bodies -- --exact --nocapture",
                1,
            ),
            valid.replacen(
                SECRET_EXPOSURE_SESSION_BOUNDARY_SMOKE_GATE,
    ...
    #[test]
    fn jwt_policy_review_runtime_smoke_rejects_omission_substitution_and_suppression() {
        let valid = include_str!("../../../.github/workflows/tests.yml").replace("\r\n", "\n");
        for mutation in [
            valid.replacen(JWT_POLICY_REVIEW_SMOKE_GATE, "", 1),
            valid.replacen(
                JWT_POLICY_REVIEW_SMOKE_GATE,
                "      - name: Exercise opt-in local JWT policy review CLI\n        run: cargo test --release --locked -p termivar-cli --all-features --test jwt_policy_cli -- --nocapture",
                1,
            ),
            valid.replacen(
                JWT_POLICY_REVIEW_SMOKE_GATE,
                &format!("{JWT_POLICY_REVIEW_SMOKE_GATE}\n        continue-on-error: true"),
                1,
            ),
        ] {
            assert_ne!(mutation, valid, "mutation must alter the workflow fixture");
            let violations =
                capabilities_workflow_policy_violations(&[(TESTS_WORKFLOW.to_owned(), mutation)]);
            assert_eq!(violations.len(), 1, "{violations:?}");
            assert!(violations[0].contains("JWT-policy"), "{violations:?}");
        }
    }

    #[test]
    fn jwt_target_acceptance_runtime_smoke_rejects_omission_widening_and_suppression() {
        let valid = include_str!("../../../.github/workflows/tests.yml").replace("\r\n", "\n");
        for mutation in [
            valid.replacen(JWT_TARGET_ACCEPTANCE_REVIEW_SMOKE_GATE, "", 1),
            valid.replacen(
                JWT_TARGET_ACCEPTANCE_REVIEW_SMOKE_GATE,
                "      - name: Exercise opt-in JWT target-acceptance review CLI\n        run: cargo test --release --locked -p termivar-cli --all-features --test jwt_policy_cli -- --nocapture",
                1,
            ),
            valid.replacen(
                JWT_TARGET_ACCEPTANCE_REVIEW_SMOKE_GATE,
    ...
    #[test]
    fn recon_ct_provider_runtime_smoke_rejects_omission_widening_and_suppression() {
        let valid = include_str!("../../../.github/workflows/tests.yml").replace("\r\n", "\n");
        for mutation in [
            valid.replacen(RECON_CT_PROVIDER_SMOKE_GATE, "", 1),
            valid.replacen(
                RECON_CT_PROVIDER_SMOKE_GATE,
                "      - name: Exercise opt-in Cert Spotter CT provider CLI\n        run: cargo test --release --locked -p termivar-cli --all-features --test recon_ct_provider_cli -- --nocapture",
                1,
            ),
            valid.replacen(
                RECON_CT_PROVIDER_SMOKE_GATE,
                &format!("{RECON_CT_PROVIDER_SMOKE_GATE}\n        continue-on-error: true"),
                1,
            ),
        ] {
            assert_ne!(mutation, valid, "mutation must alter the workflow fixture");
            let violations =
                capabilities_workflow_policy_violations(&[(TESTS_WORKFLOW.to_owned(), mutation)]);
            assert_eq!(violations.len(), 1, "{violations:?}");
            assert!(violations[0].contains("Cert Spotter"), "{violations:?}");
        }
    }

    #[test]
    fn recon_ct_provider_four_platform_capability_smoke_is_exact() {
        let valid = include_str!("../../../.github/workflows/tests.yml").replace("\r\n", "\n");
        for mutation in [
            valid.replacen(RECON_CT_PROVIDER_CAPABILITIES_SMOKE_GATE, "", 1),
            valid.replacen(
                "TERMIVAR_CAPABILITIES_MATRIX_CASE: recon-ct-provider-only",
                "TERMIVAR_CAPABILITIES_MATRIX_CASE: release-bundle",
                1,
            ...
            valid.replacen(
                RECON_CT_PROVIDER_CAPABILITIES_SMOKE_GATE,
                &RECON_CT_PROVIDER_CAPABILITIES_SMOKE_GATE.replace(
                    "--no-default-features --features recon-ct-provider --test capabilities_cli",
    ...
    #[test]
    fn recon_snapshot_import_runtime_smoke_rejects_omission_widening_and_suppression() {
        let valid = include_str!("../../../.github/workflows/tests.yml").replace("\r\n", "\n");
        for mutation in [
            valid.replacen(RECON_SNAPSHOT_IMPORT_SMOKE_GATE, "", 1),
            valid.replacen(
                RECON_SNAPSHOT_IMPORT_SMOKE_GATE,
                "      - name: Exercise opt-in reconnaissance snapshot import CLI\n        run: cargo test --release --locked -p termivar-cli --all-features --test recon_snapshot_cli -- --nocapture",
                1,
            ),
            valid.replacen(
                RECON_SNAPSHOT_IMPORT_SMOKE_GATE,
            ...
            assert_eq!(violations.len(), 1, "{violations:?}");
            assert!(
                violations[0].contains("recon-snapshot-import"),
                "{violations:?}"
            );
        }
    }
    ...
    #[test]
    fn recon_snapshot_import_four_platform_capability_smoke_is_exact() {
        let valid = include_str!("../../../.github/workflows/tests.yml").replace("\r\n", "\n");
        for mutation in [
            valid.replacen(RECON_SNAPSHOT_IMPORT_CAPABILITIES_SMOKE_GATE, "", 1),
            valid.replacen(
                "TERMIVAR_CAPABILITIES_MATRIX_CASE: recon-snapshot-import-only",
                "TERMIVAR_CAPABILITIES_MATRIX_CASE: release-bundle",
                1,
            ...
            valid.replacen(
                RECON_SNAPSHOT_IMPORT_CAPABILITIES_SMOKE_GATE,
                &RECON_SNAPSHOT_IMPORT_CAPABILITIES_SMOKE_GATE.replace(
                    "--no-default-features --features recon-snapshot-import --test capabilities_cli",
                    "--all-features --test capabilities_cli",
                ),
                1,
            ),
