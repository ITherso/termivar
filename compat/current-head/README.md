...
downstream crates compile against the Venom sources in the same checkout:

- the default, transport-neutral `venom-core` surface;
...
Each package is `publish = false`, forbids unsafe code, and binds its Venom
...
select a v1 baseline, compare two Venom releases, promise a stable Rust ABI,
