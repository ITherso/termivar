...
strict local policy, and exactly one matching credential source select one
context-isolated authenticated child inside the existing assessment. V1
...
mandatory; a policy cannot deselect one and still belong to this contract. The
revision must change with private policy semantics, while a reported SHA-256 of
the exact public key point detects changed supplied key bytes without
authenticating their source.

The closed V1 profile accepts only a bounded compact ES256 JWS and one strict
local EC/P-256/ES256 public JWK. Structural admission does not establish P-256
...
The non-default `recon-ct-provider` scanner/CLI feature remains outside
`default`, the seven-member `release-bundle`, and the published alpha.2
archives. Explicit `--profile web-review --recon-certspotter-policy FILE`
selects one strict `security.recon-certspotter-policy/v1` file. Both
`provider_use_authorized` and `privacy_disclosure_acknowledged` must be true.

The `production` policy token names only the fixed live-endpoint mode at
`https://api.certspotter.com/v1/issuances`; it does not establish provider
approval, account entitlement, or general production entitlement. The required
`provider_use_authorized` acknowledgement records the operator's decision that
the query fits the provider-documented limited unauthenticated
personal/evaluation-use basis, hourly quota, and current terms. The only
alternate route is an explicit operator-owned numeric-loopback fixture. The
child uses the shared parent request, byte, deadline, and cancellation authority.
Only complete supported page bodies enter hypothesis parsing. Every dispatched
attempt and terminal outcome, including throttling, unsupported responses, and
transport failure, remains bounded accounting in the runtime audit and final
report. It dispatches at most two sequential pages,
retains/interprets at most 256 KiB per page and 512 KiB total, retains at most
256 names, and performs no retries or polling. V1 supplies no credentials.
Redirects and ambient proxies are disabled.

Returned CT names stay source-qualified hypotheses and never become scan
subjects, broker permits, scan authority, ownership or authentication evidence,
assessment items, or findings. Release-candidate acceptance does not receive a
production query/authorization input and therefore records live service status
`NOT_RUN_NO_INPUT`. See
[Cert Spotter CT reconnaissance provider](recon-certspotter-provider.md).
...
There is no separate WordPress scanner, provider, budget, queue, or finalizer.
Enabling only the review adds zero target/provider requests and zero active
verifications. One `technology.wordpress-surface-observed@1` `Informational` /
`KnowledgeOnly` item may describe supported root-response hints.

The additional `--wordpress-discovery` opt-in reuses the same assessment broker,
budget, evidence lifecycle, and deadline for at most twelve anonymous, bodyless,
same-origin metadata GET attempts. Without a page scope, candidates still come
only from committed root evidence. Explicit `--wordpress-page-scope observed`
may additionally interpret at most three eligible anonymous complete HTML GET
responses already committed by the ordinary assessment; it dispatches no page
request, while accepted component hints may nominate metadata requests inside
the same twelve-attempt allowance. It does not use credentials, follow
redirects, retry, seed general crawling, or add an active verification. A separate
`technology.wordpress-metadata-source-response-observed@1` informational
observation may retain references to committed REST-index, theme-stylesheet, or
...
live installation, prove advisory applicability, execute an exploit, or validate
impact. The feature remains outside `default`. The current untagged alpha.3
`release-bundle` compiles this Preview, but it still adds no work without
explicit CLI selection; published alpha.2 archives predate all WordPress
producer and audit support.
...
> the published `v0.10.0-alpha.2` prerelease does not include later `main`
...
The [OAST correlation foundation](oast-correlation.md) is a non-default,
transport-free `termivar-scanner` library surface. It accepts host-minted
move-only correlation tokens and already reduced DNS/HTTP events under exact
verification-case and bounded polling contracts. It owns no provider adapter,
listener, clock, random generator, CLI flag, scanner action, target request,
...
bounded raw-free events, and performs explicit cleanup. The non-default
[native OAST provider authority](native-oast-provider-authority.md) adds one
...
replay through fresh ambient-proxy-free connection pools governed by the parent
broker's exact-origin policy, budget, cancellation and evidence authority. All
four legs are one logical active verification, charged when the primary replay
begins; the peer replay is passive-accounted inside the same active decision
phase. Credentials, query values, the clear resource handle,
...
- **Local JWT policy review** (`jwt-policy-review`, opt-in) consumes one
  explicitly supplied compact token, strict local policy with mandatory intended
  `typ`, issuer and audience bindings, and a local ES256 public
...
| Cert Spotter CT reconnaissance provider | scanner and CLI opt-in (`recon-ct-provider`) plus explicit `--profile web-review --recon-certspotter-policy FILE` and both policy acknowledgements | the `production` token selects only the fixed live endpoint, or an owned numeric-loopback fixture; the child shares parent request/byte/deadline/cancellation authority, and complete responses enter bounded runtime audit/composition while names remain inert source-qualified hypotheses | no | Preview, development-only; unauthenticated use is limited to the provider-documented personal/evaluation basis, hourly quota and current terms, and acknowledgement does not prove provider approval or account/general-production entitlement; at most two sequential pages, 256 KiB/page, 512 KiB total and 256 retained names; no retries, polling or V1 credentials; never scan authority/ownership/authentication/findings; live service acceptance `NOT_RUN_NO_INPUT`; outside `release-bundle` |
...
20 known feature identities: the marker plus seven compiled members and twelve
...
`wordpress-review`. The twelve excluded identities are `api-adapter`,
