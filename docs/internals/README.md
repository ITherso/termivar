...
- [Passive response secret-exposure review](passive-secret-exposure-review.md): explicit anonymous opt-in, zero-additional-request admission over committed complete textual GET bodies, a fixed bounded detector catalogue, and value-free reporting.
- [Existing-connection TLS observation](existing-connection-tls-observation.md): explicit passive opt-in over successful responses already obtained by the broker, bounded value-free leaf-certificate facts, and honest backend unknowns without an extra request or handshake.
- [Local JWT and optional target-acceptance review](local-jwt-policy-review.md): strict local ES256 policy evaluation plus a separate six-leg exact-resource target child, with guarded secret intake, value-free evidence, shared parent accounting, and no Confirmed claim.
...
- [Normalization-resilience review](normalization-resilience.md): explicit opt-in, candidate-specific defense eligibility, typed depth-one HTML transforms, replay, and conservative claim authority.
