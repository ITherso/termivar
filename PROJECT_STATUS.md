...
- The default `venom scan` command exercises the conservative deterministic single-resource runtime; its operational outcomes are not findings or vulnerability verdicts. Explicit `web-review` adds bounded exact-origin discovery plus passive and matched low-risk review under the same host-owned authority. Its native actions are KnowledgeOnly and can produce at most `NeedsReview`. The historical heuristic runner is separately feature-gated and requires explicit acknowledgement. Its phases 2–4 share bounded passive discovery and phases 5–9 share a distinct bounded active-verification authority. Phase one and custom extensions can still perform raw I/O, so whole-run accounting remains `Unmetered`; the narrower phase-5-to-9 authority must not be mistaken for `RuntimeBudget` coverage.
