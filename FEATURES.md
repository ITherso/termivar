...
| Source coverage | Calibration automated | Pinned Tarpaulin emits Cobertura and deterministic repository-owned summaries; CI attempts a best-effort advisory Codecov upload whose tokenless availability is not enforced, changed files and exact path/blob-stable omissions are inventoried fail-closed, and no accepted numeric floor is committed yet |
