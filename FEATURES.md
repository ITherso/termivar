...
| Native low-risk web review | Preview | Explicit `web-review` only. Matched CORS, optional allowlisted redirect/reflection, one bounded SQL quote-balance review, and one initial versioned SSTI arithmetic probe family run through the existing Standard runtime and shared broker. SQL and SSTI select at most one deterministic parameter; SSTI requires exact candidate-specific arithmetic results across independent matched replay, while literal reflection, static output, generic errors, and incomplete evidence cannot create an item. Redirects are never followed, and no native action can confirm a vulnerability |
