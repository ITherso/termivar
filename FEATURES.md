...
| Historical scanner salvage ledger | Preview | Repository-only `venom.historical-scanner-salvage/v1` metadata binds the deleted 38-file tree to local Git blobs and component-level dispositions. It is not runtime authority. The detector byte-pattern component is restored by `venom-artifact`, the sanitized request/response fixture component is restored as repository-only conformance data, and only the bounded GraphQL subset of the protocol taxonomy is restored by GraphQL review; unsafe mmap, synthetic vulnerabilities, and fabricated legacy finding logic remain rejected |
