...
| Scanner conformance corpus | Preview, repository-only | A strict `security-assessment-fixture/v1` corpus holds 73 sanitized request/response cases, including 23 four-view authorization differentials, with typed expectations and deterministic `xtask scanner-corpus` validation. It changes no runtime action, request, default feature, or report claim; passing the corpus is contract evidence, not an empirical scanner-accuracy result |
