...
| Exploit manifest/catalog foundation | Preview | Independent `venom-exploit` library validates bounded versioned metadata, applicability/risk declarations, authorization requirements, lifecycle transitions, and deterministic catalogs. It has no executor, real exploit module, transport/process/browser/filesystem authority, scanner/CLI integration, or automatic AssessmentItem transition; a manifest is never authorization |
