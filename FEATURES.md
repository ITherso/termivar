...
| Historical scanner salvage ledger | Preview | Repository-only `venom.historical-scanner-salvage/v1` metadata binds the deleted 38-file tree to local Git blobs and component-level dispositions. It is not runtime authority. Only the detector byte-pattern component is marked restored by `venom-artifact`; unsafe mmap and fabricated legacy finding logic remain rejected |
