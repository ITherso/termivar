...
| Native low-risk web review | Preview | Explicit `web-review` only. Matched CORS, optional allowlisted redirect, one bounded scanner-marker reflection pair, one SQL quote-balance review, and one initial versioned SSTI arithmetic family run through the existing Standard runtime and shared broker. The bounded `html5ever` reflection classifier parses supported HTML once into typed contexts; it performs no escape testing, JavaScript/CSS analysis, browser execution, or XSS confirmation. Redirects are never followed, and no native action can confirm a vulnerability |
