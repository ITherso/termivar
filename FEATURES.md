...
| Native low-risk web review | Preview | Explicit `web-review` only. Matched CORS, optional allowlisted redirect, one bounded scanner-marker reflection pair, one SQL quote-balance review, and one initial versioned SSTI arithmetic family run through the existing Standard runtime and shared broker. The bounded `html5ever` reflection classifier feeds a typed, versioned four-family XSS structural catalog; V1 executes only the HTML-text family whose exact inert node boundary can be proved, while URI/handler/script families remain metadata-only pending stronger evidence support. Candidate-specific structural control is `NeedsReview` only—there is no JavaScript, event, navigation, browser execution, or XSS confirmation. Redirects are never followed, and no native action can confirm a vulnerability |
