...
| Post-workspace WAF/evasion salvage ledger | Preview | A separate repository-only `venom.post-workspace-waf-evasion-salvage/v1` epoch binds 13 historical files and 39 classified components to the quarantine transition. It maps superseded fingerprinting and neutral encoding to current replacements, preserves recoverable transformation concepts as metadata, and rejects blind dispatch, misleading status claims, and unsafe request framing. It restores no source and creates no runtime, request, execution, or claim authority |
