...
| Artifact signature scanning | Preview | Independent `venom-artifact` scans bounded caller-supplied buffers/readers for exact/wildcard signatures and emits deterministic observations. Explicit local regular-file access requires the non-default CLI `artifact-adapter`; there is no recursion, process-memory acquisition, network path, execution, malware verdict, or vulnerability claim |
| Historical scanner salvage ledger | Preview | Repository-only `venom.historical-scanner-salvage/v1` metadata binds the deleted 38-file tree to local Git blobs and component-level dispositions. It is not runtime authority. The detector byte-pattern component is restored by `venom-artifact`, the sanitized request/response fixture component is restored as repository-only conformance data, and bounded GraphQL plus OpenAPI/REST contract subsets have been independently rebuilt while the broader protocol-taxonomy component remains planned; unsafe mmap, synthetic vulnerabilities, and fabricated legacy finding logic remain rejected |
| Post-workspace WAF/evasion salvage ledger | Preview | A separate repository-only `venom.post-workspace-waf-evasion-salvage/v1` epoch binds 13 historical files and 39 classified components to the quarantine transition. It maps superseded fingerprinting and neutral encoding to current replacements, records only the typed HTML token-case and inter-token-tab concepts as restored by normalization resilience, and keeps blind dispatch, misleading status claims, generic mutation, and unsafe request framing rejected |
| Scanner conformance corpus | Preview, repository-only | A strict `security-assessment-fixture/v1` corpus holds 103 sanitized request/response cases, including 23 four-view authorization differentials and 30 bounded OpenAPI contract cases, with typed expectations and deterministic `xtask scanner-corpus` validation. It changes no runtime action, request, default feature, or report claim; passing the corpus is contract evidence, not an empirical scanner-accuracy result |
...
| Normalization-resilience review | Preview, opt-in | Non-default scanner/CLI feature plus explicit `--normalization-resilience` on `web-review`. V1 selects at most one depth-one typed HTML transform (`xss.html-token-case@1` or `xss.html-inter-token-tab@1`), reuses committed parent XSS evidence, and spends at most three child requests/one active verification on candidate plus replay. A result requires the same inert application semantics and remains `NeedsReview` / `KnowledgeOnly`; percent decode-depth families are metadata-only, and fingerprints are hints rather than authority |
...
| JSON/GraphQL reasoning | Preview | Opt-in deterministic, transport-neutral fingerprinting; paired differences remain review hypotheses, not vulnerability verification |
| GraphQL surface review | Preview, opt-in | Non-default scanner/CLI `graphql-review` feature plus explicit `--graphql-review` on `web-review`. V1 selects one exact-origin endpoint at most and sends up to three anonymous POST/JSON requests: aliased `__typename`, bounded schema-root introspection, and a distinct active-verification replay. Fail-closed protocol outcomes stop the sequence early. Surface and replayed introspection observations are `Informational` / `KnowledgeOnly`; there is no mutation, full schema enumeration, credential use, authorization/DoS claim, or WebSocket transport |
| OpenAPI surface review | Preview, opt-in | Non-default scanner/CLI `openapi-review` feature plus explicit `--openapi-review` on `web-review`. V1 selects at most one exact-origin document from bounded committed discovery or the single `/openapi.json` fallback and dispatches one anonymous bodyless GET plus one exact replay through the shared broker. Only bounded OpenAPI 3.0/3.1 JSON can yield an `Informational` / `KnowledgeOnly` contract observation; YAML is unsupported and Swagger 2.0 remains metadata-only |
...
| Resource authorization review | Preview, opt-in | Non-default scanner/CLI `authorization-review` feature. One strict `security.authorization-review-policy/v1` file and distinct primary/peer credentials from the existing env/file/stdin boundary select one exact-origin JSON `GET` resource. One native action uses four ordered legs and one logical active verification through the parent assessment broker and budget. Only independently replay-stable equivalence across Status, Fields, and Resources may emit `authorization.resource-cross-principal-equivalence@1`, capped at `NeedsReview` / `KnowledgeOnly`; there is no identifier mutation, enumeration, write method, cookie, or confirmed authorization claim |
